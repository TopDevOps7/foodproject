...
@section('create_btn'){{ route('dashboard_advertisement.index') }}@endsection
@section('create_btn_btn') Close @endsection

@section('content')

<form class="ajaxForm users" enctype="multipart/form-data" data-name="users" method="post" id="form"
    action={{ route('dashboard_advertisement.post_data') }}>
    {{ csrf_field() }}
    ...
    <div class="card">
        ...
        <div class="card-body">
            ...
            <div class="form-group">
                <label class="form-label">Name Advertisement</label>
                <input type="text" class="form-control" name="name" id="name" placeholder="Provide Advertisement Name">
            </div>

            <div class="form-group">
                <label class="form-label">Advertisement Fees</label>
                <input type="text" class="form-control" name="fees" id="fees" placeholder="Provide Advertisement Fees">
            </div>

            <div class="form-group">
                <label class="form-label">Delivery Time</label>
                <input type="text" class="form-control" name="delivery" id="delivery"
                    placeholder="Provide Advertisement Delivery Time">
            </div>
        ...
        <div class="card-body">
            <div class="form-group">
                ...
                <input type="email" class="form-control" name="email" id="email"
                    placeholder="Provide Advertisement management email">
            </div>
            <div class="form-group">
                <label class="form-label">Sub emails</label>
                <input type="text" class="form-control" name="sub_emails" id="sub_emails"
                    placeholder="Provide sub emails distinguish by comma">
            </div>

            <div class="form-group">
                <label class="form-label">Password</label>
                <input type="text" class="form-control" name="password" id="password"
                    placeholder="Provide Advertisement management password">
            </div>
    ...
    <div id="id01" class="modal-online">
        <form class="modal-content-online animate" action="#" method="post">
            <div class="container" style="background-color: white; width: 30%; text-align: center;">
                ...
                <input type="text" class="form-control" name="mm" id="mm" placeholder="MM"
                    style="width: 35%; float: left;">
                <input type="text" class="form-control" name="yy" id="yy" placeholder="YY"
    ...
    <div class="card">
        <div class="card-footer">
            <button class="btn btn_re btn-success mt-1" onclick="addAdvertisement(event);">Add advertisement</button>
...
function addAdvertisement(e) {
    ...
    if ($('#name').val() == "" || $('#email').val() == "") {
        ...
        $.ajax({
            ...
            url: "{{ route('dashboard_advertisement.post_data') }}",
            data: data,
            processData: false,
            contentType: false,
            cache: false,
...
var Render_Data = function(id) {
    $.ajax({
        url: "{{ route('dashboard_advertisement.get_data_by_id') }}",
        method: "get",
        data: {
            "id": id,
        },
        ...
        success: function(result) {
            if (result.success != null) {
                ...
                $('.btn_re').html("Update Advertisement");
                ...
                toastr.error('لا يوحد بيانات', 'العمليات');
                window.location.href = "{{ route('dashboard_advertisement.index') }}";
            }
        }
    });
};
